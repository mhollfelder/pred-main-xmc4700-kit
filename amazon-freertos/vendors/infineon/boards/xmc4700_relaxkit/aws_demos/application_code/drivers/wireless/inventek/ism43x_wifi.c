...
static ISM43x_WIFI_SecurityType_t prvParseSecurity( char* pcPtr );
static void prvAT_ParseConnSettings( char *pcData, ISM43x_WIFI_Network_t *pxNetSettings );
static void prvAT_ParseIsConnected( char *pcData, uint8_t *pucIsConnected );
static ISM43x_WIFI_Status_t prvAT_ExecuteCommand( ISM43x_WIFIObject_t *pxObj );
...
static ISM43x_WIFI_SecurityType_t prvParseSecurity( char* pcPtr )
{
  if( strstr(pcPtr,"Open") )            return ISM43x_WIFI_SEC_OPEN;
  else if( strstr(pcPtr,"WEP") )        return ISM43x_WIFI_SEC_WEP;
  else if( strstr(pcPtr,"WPA WPA2") )   return ISM43x_WIFI_SEC_WPA_WPA2;
  else if( strstr(pcPtr,"WPA2 TKIP") )  return ISM43x_WIFI_SEC_WPA2_TKIP;
  else if( strstr(pcPtr,"WPA2") )       return ISM43x_WIFI_SEC_WPA2;
  else if( strstr(pcPtr,"WPA") )        return ISM43x_WIFI_SEC_WPA;
  else                                  return ISM43x_WIFI_SEC_UNKNOWN;
}
...
/**
  ...
  * @retval None.
  */
static void AT_ParseInfo( ISM43x_WIFIObject_t *xObj,uint8_t *pData )
{
    char *pcPtr;
    uint8_t ucNum = 0;
    ...
    while( pcPtr != NULL )
    {
        ...
        {
            case 0:
                strncpy( (char *)xObj->Product_ID,  pcPtr, ISM43x_WIFI_PRODUCT_ID_SIZE );
                break;

            case 1:
                strncpy( (char *)xObj->FW_Rev,  pcPtr, ISM43x_WIFI_FW_REV_SIZE );
                break;

            case 2:
                strncpy( (char *)xObj->API_Rev,  pcPtr, ISM43x_WIFI_API_REV_SIZE );
                break;

            case 3:
                strncpy( (char *)xObj->Stack_Rev,  pcPtr, ISM43x_WIFI_STACK_REV_SIZE );
                break;

            case 4:
                strncpy( (char *)xObj->RTOS_Rev,  pcPtr, ISM43x_WIFI_RTOS_REV_SIZE );
                break;
            ...
            case 6:
                ...
                strncpy( (char *)xObj->Product_Name,  pcPtr, ISM43x_WIFI_PRODUCT_NAME_SIZE );
                break;
...
/**
  ...
  * @retval None.
  */
static void prvAT_ParseAP( char *pcData, ISM43x_WIFI_APs_t *pxAPs )
{
    uint8_t ucNum = 0;
    char *pcPtr;
    pxAPs->nbr = 0;
    ...
    while( (pcPtr != NULL) && (pxAPs->nbr < ISM43x_WIFI_MAX_DETECTED_AP) )
    {
        switch (ucNum++)
        {
            case 0: /* Ignore index */
            ...
            case 1:
                ...
                strncpy( (char *)pxAPs->AP[pxAPs->nbr].SSID,  pcPtr+ 1, ISM43x_WIFI_MAX_SSID_NAME_SIZE + 1 );
                break;
...
/**
  ...
  * @retval None.
  */
static void prvAT_ParseSingleAP( char *pcData, ISM43x_WIFI_AP_t *pxAP )
{
    uint8_t ucNum = 0;
    char *pcPtr;
    ...
    while( pcPtr != NULL )
    {
        switch( ucNum++ )
        {
            ...
            case 1:
                pcPtr[strlen(pcPtr) - 1] = 0;
                strncpy( (char *)pxAP->SSID,  pcPtr+ 1, ISM43x_WIFI_MAX_SSID_NAME_SIZE + 1 );
                break;
...
static void prvAT_ParseConnSettings(char *pcData, ISM43x_WIFI_Network_t *pxNetSettings)
{
    ...
    while( pcPtr != NULL )
    {
        ...
        {
            case 0:
                strncpy( (char *)pxNetSettings->SSID,  pcPtr, ISM43x_WIFI_MAX_SSID_NAME_SIZE + 1 );
                break;

            case 1:
                strncpy( (char *)pxNetSettings->pswd,  pcPtr, ISM43x_WIFI_MAX_PSWD_NAME_SIZE + 1 );
                break;

            case 2:
                pxNetSettings->Security = (ISM43x_WIFI_SecurityType_t)prvParseNumber( pcPtr, NULL );
                break;
            ...
            case 4:
                pxNetSettings->IP_Ver = (ISM43x_WIFI_IPVer_t)prvParseNumber( pcPtr, NULL );
                break;
...
static ISM43x_WIFI_Status_t prvAT_ExecuteCommand( ISM43x_WIFIObject_t *pxObj )
{
    ...
    if( sRet > 0 )
    {
        sRecvLen = pxObj->fops.IO_Receive( pxObj->RxTxData, ISM43x_WIFI_DATA_SIZE, pxObj->Timeout );
        pxObj->RxTxDataLength = sRecvLen;
        if( (sRecvLen > 0) && ( sRecvLen < ISM43x_WIFI_DATA_SIZE) )
        {
            *(pxObj->RxTxData + sRecvLen) = 0;
            if( strstr( (char *)pxObj->RxTxData, AT_OK_STRING ) )
            {
                UNLOCK_WIFI();
                return ISM43x_WIFI_STATUS_OK;
            }
            else if( strstr( (char *)pxObj->RxTxData, AT_ERROR_STRING ) )
            {
                UNLOCK_WIFI();
                return ISM43x_WIFI_STATUS_UNEXPECTED_CLOSED_SOCKET;
            }
        }
        if(sRecvLen == ISM43x_WIFI_ERROR_STUFFING_FOREVER )
        {
            UNLOCK_WIFI();
            return ISM43x_WIFI_STATUS_MODULE_CRASH;
        }
    }
    UNLOCK_WIFI();  
    return ISM43x_WIFI_STATUS_IO_ERROR;
}
...
/**
  ...
  * @retval Operation Status.
  */
static ISM43x_WIFI_Status_t AT_RequestReceiveData( ISM43x_WIFIObject_t *pxObj, uint8_t* pucCmd, char *pcData, uint16_t usReqlen, uint16_t *pusReadData )
{
    int16_t sLen;
    ...
    if( pxObj->fops.IO_Send( pucCmd, strlen( (char*)pucCmd ), pxObj->Timeout ) > 0 )
    {
        ...
        if( sLen == ISM43x_WIFI_ERROR_STUFFING_FOREVER )
        {
            UNLOCK_WIFI();
            return ISM43x_WIFI_STATUS_MODULE_CRASH;
        }
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_Init( ISM43x_WIFIObject_t *pxObj )
{
    ISM43x_WIFI_Status_t xRet = ISM43x_WIFI_STATUS_ERROR;

    pxObj->Timeout = ISM43x_WIFI_TIMEOUT;

    if( pxObj->fops.IO_Init(ISM43x_WIFI_INIT) == 0 )
    {
        LOCK_WIFI();
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_DeInit( ISM43x_WIFIObject_t *pxObj )
{
    ISM43x_WIFI_Status_t xRet = ISM43x_WIFI_STATUS_OK;
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_GetProductID( ISM43x_WIFIObject_t *pxObj, uint8_t *pucPoductID )
{
    strncpy( (char *)pucPoductID, (char *)pxObj->Product_ID, ISM43x_WIFI_PRODUCT_ID_SIZE );
    return ISM43x_WIFI_STATUS_OK;
}
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_GetFWRevID( ISM43x_WIFIObject_t *pxObj, uint8_t *pucFWRev )
{
    strncpy( (char *)pucFWRev, (char *)pxObj->FW_Rev, ISM43x_WIFI_FW_REV_SIZE );
    return ISM43x_WIFI_STATUS_OK;
}
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_GetProductName( ISM43x_WIFIObject_t *pxObj, uint8_t *pucProductName )
{
    strncpy((char *)pucProductName, (char *)pxObj->Product_Name, ISM43x_WIFI_PRODUCT_NAME_SIZE);
    return ISM43x_WIFI_STATUS_OK;
}
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_GetAPIRev( ISM43x_WIFIObject_t *pxObj, uint8_t *pucAPIRev )
{
    strncpy( (char *)pucAPIRev, (char *)pxObj->API_Rev, ISM43x_WIFI_API_REV_SIZE );
    return ISM43x_WIFI_STATUS_OK;
}
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_GetStackRev( ISM43x_WIFIObject_t *pxObj, uint8_t *pucStackRev )
{
    strncpy( (char *)pucStackRev, (char *)pxObj->Stack_Rev, ISM43x_WIFI_STACK_REV_SIZE );
    return ISM43x_WIFI_STATUS_OK;
}
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_GetRTOSRev( ISM43x_WIFIObject_t *pxObj, uint8_t *pucRTOSRev )
{
    strncpy( (char *)pucRTOSRev, (char *)pxObj->RTOS_Rev, ISM43x_WIFI_RTOS_REV_SIZE );
    return ISM43x_WIFI_STATUS_OK;
}
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_RegisterBusIO( ISM43x_WIFIObject_t *pxObj, IO_Init_Func pcIO_Init,
                                                              ...
                                                              IO_Receive_Func  psIO_Receive )
{
    ...
    {
        return ISM43x_WIFI_STATUS_ERROR;
    }
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_SetTimeout( ISM43x_WIFIObject_t *pxObj, uint32_t ulTimeout )
{
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t  ISM43x_WIFI_ListAccessPoints( ISM43x_WIFIObject_t *pxObj, ISM43x_WIFI_APs_t *pxAPs )
{
    ISM43x_WIFI_Status_t xRet;
    int16_t sSendLen;
    int16_t sRecvLen = 0;
    ...
    if( prvReverseByteOrder32bit(pucVersion) >= UPDATED_SCAN_PARAMETERS_FW_REV )
    {
        ...
        if( sSendLen == 5 )
        {
            do
            {
                ...
                if( pxAPs->nbr < ISM43x_WIFI_MAX_DETECTED_AP )
                {
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_Connect( ISM43x_WIFIObject_t *pxObj, const char* pcSSID,
                                         const char* pcPassword,
                                         ISM43x_WIFI_SecurityType_t xSecType )
{
    ISM43x_WIFI_Status_t xRet;
    ISM43x_WIFI_NetworkAutoConnect_t auto_mode = ISM43x_WIFI_AUTO_JOIN_RECONNECT;
...
/**
  ...
  * @retval Operation Status.
  */
int32_t ISM43x_WIFI_GetRssi( ISM43x_WIFIObject_t *pxObj )
{
    ISM43x_WIFI_Status_t xRet ;
    LOCK_WIFI();
    int32_t slRssi = 0;
...
/**
  ...
  * @retval Operation Status.
  */
uint8_t ISM43x_WIFI_IsConnected( ISM43x_WIFIObject_t *pxObj )
{
    ISM43x_WIFI_Status_t xRet ;
    LOCK_WIFI();
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_Disconnect( ISM43x_WIFIObject_t *pxObj )
{
    ISM43x_WIFI_Status_t xRet;
    ...
    ISM43x_WIFI_SetTimeout( pxObj, 1000 );// don't wait for disconnect too much.
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_GetNetworkSettings( ISM43x_WIFIObject_t *pxObj )
{
    ISM43x_WIFI_Status_t xRet;
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_ActivateAP( ISM43x_WIFIObject_t *pxObj, ISM43x_WIFI_APConfig_t *pxApConfig )
{
    ISM43x_WIFI_Status_t xRet;
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_GetMACAddress(ISM43x_WIFIObject_t *pxObj, uint8_t *pucMac)
{
    ISM43x_WIFI_Status_t xRet ;
    char *pcPtr;
    LOCK_WIFI();
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_Ping( ISM43x_WIFIObject_t *pxObj, uint8_t *pucAddress, uint16_t usCount, uint16_t usIintervalMs )
{
    ISM43x_WIFI_Status_t xRet;
            ...
            if( xRet == ISM43x_WIFI_STATUS_OK )
            {
                ...
                if( xRet == ISM43x_WIFI_STATUS_OK )
                {
                        ...
                        */
                        ...
                        xRet = ISM43x_WIFI_STATUS_TIMEOUT;
                        do
                        {
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_DNS_LookUp( ISM43x_WIFIObject_t *pxObj, const char *pcUrl, uint8_t *pucIpAddress )
{
    ISM43x_WIFI_Status_t xRet;
    char *pcPtr;
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_StartClientConnection( ISM43x_WIFIObject_t *pxObj, ISM43x_WIFI_Conn_t *pxConn )
{
    ISM43x_WIFI_Status_t xRet = ISM43x_WIFI_STATUS_OK;
    ...
    if( ( xRet == ISM43x_WIFI_STATUS_OK ) && ( ( pxConn->Type == ISM43x_WIFI_TCP_CONNECTION ) || ( pxConn->Type == ISM43x_WIFI_TCP_SSL_CONNECTION ) ) )
    {
    ...
    if( ( xRet == ISM43x_WIFI_STATUS_OK ) && ( pxConn->Type == ISM43x_WIFI_TCP_SSL_CONNECTION ) )
    {
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_StopClientConnection( ISM43x_WIFIObject_t *pxObj, ISM43x_WIFI_Conn_t *pxConn )
{
    ISM43x_WIFI_Status_t xRet;
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_StartServerSingleConn( ISM43x_WIFIObject_t *pxObj, ISM43x_WIFI_Conn_t *pxConn )
{
    ISM43x_WIFI_Status_t xRet = ISM43x_WIFI_STATUS_OK;
    ...
    if( ( pxConn->Type != ISM43x_WIFI_UDP_CONNECTION ) && ( pxConn->Type != ISM43x_WIFI_UDP_LITE_CONNECTION ) )
    {
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_SendData( ISM43x_WIFIObject_t *pxObj, uint8_t ucSocket, uint8_t *pucData, uint16_t usReqlen , uint16_t *pusSentLen , uint32_t ulTimeout )
{
    ISM43x_WIFI_Status_t xRet = ISM43x_WIFI_STATUS_ERROR;

    LOCK_WIFI();

    if( usReqlen >= ISM43x_WIFI_PAYLOAD_SIZE )
    {
        usReqlen = ISM43x_WIFI_PAYLOAD_SIZE;
    }
    ...
    *pusSentLen = usReqlen;
    ...
    if( xRet == ISM43x_WIFI_STATUS_ERROR )
    {
        *pusSentLen = 0;
    }
...
/**
  ...
  * @retval Operation Status.
  */
ISM43x_WIFI_Status_t ISM43x_WIFI_ReceiveData( ISM43x_WIFIObject_t *pxObj, uint8_t ucSocket, uint8_t *pucData, uint16_t usReqlen, uint16_t *pusReceivedlen, uint32_t ulTimeout )
{
    ISM43x_WIFI_Status_t xRet = ISM43x_WIFI_STATUS_ERROR;
        ...
        if( xRet == ISM43x_WIFI_STATUS_OK )
        {
            ...
            if( xRet == ISM43x_WIFI_STATUS_OK )
            {
                ...
                if( xRet == ISM43x_WIFI_STATUS_OK )
                {
                    ...
                    if( xRet != ISM43x_WIFI_STATUS_OK )
                    {
                        configPRINTF( ("ISM43x_WIFI_ReceiveData failed\n") );
                    }
                }
                else
                {
